import { DealPassport } from '../../../lib/api/calc'; // Assumed path
...
export class CustomsValueCalculatorV2 {
  ...
  async calculate(
    ...
    // --- Border Freight ---
    if (rule.customs_components_in_base.includes('border_freight')) {
        const inclusion = await this.resolveInclusion(passport, 'invoice_includes_border_freight', 'border_freight');
        ...
        if (inclusion === 'no') {
            if (freightToBorderUSD !== null) {
                ...
                // Missing rate!
                if (rule.critical_components.includes('border_freight')) {
                    // Critical missing
            ...
            } else if (rule.unknown_policy === 'INCOMPLETE_ONLY') {
                 result.is_incomplete = true;
                 result.missing_inputs.push('invoice_includes_border_freight');
            } else {
                // Escalate
                result.missing_inputs.push('invoice_includes_border_freight'); // Treat as missing for strictness
            }
        }
    ...
    // --- Insurance ---
    if (rule.customs_components_in_base.includes('insurance')) {
        const inclusion = await this.resolveInclusion(passport, 'invoice_includes_insurance', 'insurance');
        
        if (inclusion === 'no') {
             // Calculate Insurance
             const insRule = await this.ruleRepo.getInsuranceRule(passport.dest_country, passport.incoterms);
             const val = this.calculateInsurance(insRule, passport.goods_value, freightToBorderUSD || 0); // Logic: if freight missing, calc on invoice only?
  ...
  private async resolveInclusion(
      ...
      // 1. User Input (if override allowed - assume yes for MVP passport)
      const userValue = passport[field];
      if (userValue === 'yes' || userValue === 'no') return userValue;
      if (userValue === 'unknown') return 'unknown'; // Explicit unknown

      // 2. Default Rule
      const rule = await this.ruleRepo.getInclusionRule(passport.incoterms, component);
      
      // If user provided nothing (undefined), use default
      if (!userValue) {
          return rule.default_included; 
      }
